function create_scorecard( scorecard_template, all_doctors, callback ){
	...
	doctors.forEach( function( d ){
		...
		dateFormat(new Date(), "yyyy-mm-dd_HH:MM:ss.l") +
		"_" + doc_data.header.export_as + ".xml";
		var scorecardContents = js2xmlparser( 'ScoreCard', scorecard_json );
		...
		scpClient.defaults({
			...
			privateKey: require('fs').readFileSync(uploadConfig.raw.privateKeyPath)
		});

		// send Scorecard text
		scpClient.write({
			destination: uploadConfig.raw.destinationPath + filename,
