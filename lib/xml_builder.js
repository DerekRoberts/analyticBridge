function create_scorecard(scorecard_template, all_doctors, callback) {
    ...
    // populate Patient Counts in JSON scorecard object
		...
		contact_count_query_results
			.forEach(function(result) {
			    // Pull age category tag name from config file
			    var age_category_tag = queries.raw.ContactCounts.map[result.agesMin];
			    var destination_parent = scorecard_json.ContactCounts[result.gender][result.category];
