...
function test( scorecard_template, all_doctors ){
	...
	doctors.forEach( function( d ){
		...
		ratio_query_titles.forEach( function ( ratio_query_title ) {
			...
			ratio_results.forEach( function ( result ) {

				if(result.result == 'numerator') {
					if( !numerator) {
						numerator = result.value;
					} else {
						throw new Error( "Multiple numerator resutls for " + ratio_query_title);
					}
				}

				if(result.result == 'denominator') {
					if( !denominator) {
						denominator = result.value;
					} else {
						throw new Error( "Multiple denominator resutls for " + ratio_query_title);
					}
				}
			});

			if( !numerator) {
				throw new Error( "No numerator result for " + ratio_query_title);
			}

			if( !denominator) {
				throw new Error( "No denominator result for " + ratio_query_title);
			}
