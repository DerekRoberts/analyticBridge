 ...
 *            {object} scorecardTemplate JSON scorecard template from which to
 *            start
 * @param doctor
 *            {string} doctor CPS Id of doctor for which to create the scorecard
 * @param period
 *            {object} period Time period containing start and end moments for
 *            scorecard
 * @param data
 ...
 *            execution time used to populate this scorecard. This value may be
 *            set and the container allows it to be passed back to the calling
 *            function.
 * @returns {object} Created AMCARE scorecard object, or null if a scorecard
 *          could not be created for the values passed
 */
function createScorecard(scorecardTemplate, doctor, period, data, earliestExecutionHolder) {
    ...
    var ratioSuccess = populateRatioQueries(scorecard.ReportingCategories, doctor, period, data, earliestExecutionHolder);
    var patientCountsSuccess = populateTenYearRanges(scorecard.PatientCounts, "PatientCounts", doctor, period, data, earliestExecutionHolder);
    var contactCountsSuccess = populateTenYearRanges(scorecard.ContactCounts, "ContactCounts", doctor, period, data, earliestExecutionHolder);
...
function populateScorecardHeader(toPopulate, doctor, period, data) {
    ...
    // to this date
    var reportingDate = new moment(period.end).add(1, "days");
    toPopulate['ScoreCardDataDateTime'] = formatMomentScorecard(reportingDate);
...
function extractResult(queryTitle, type, doctorPID, period, data) {
	if (!((type === "Ratio") || (type === "TenYearRanges"))) {
		throw "Invalid value for type of \"" + type
		        + "\" passed to extractResult";
	}
	...
	if (!queryInfo.exclude && data[queryTitle]) {
		// There is results for the query that we are searching for and it is not excluded

		// Executions are probably already in ascending order by time.
		// Build new array and sort to ensure this is the case.
		// This is done in a local copy of the array as the data object MUST NOT
		// be modified
		// as it is being accessed by multiple threads.
		// An obvious performance enhancement would be to do this once in
		// advance
		timingStart = moment();
		...
		var chosenResult;
		var resultKeyObject
		// Iterate through executions in reverse order until we find our chosen
		// result.
		// If we find a result within the time period in a more recent
		// execution, do
		// not use results from older executions.
		for (var executionCtr = executions.length - 1; !chosenResult
		        && executionCtr >= 0; executionCtr--) {
			// Make sure the execution contains results
			if (typeof executions[executionCtr].aggregate_result == 'object') {
				// Build map from dataDate to numerator denominator pairs for
				// doctor
				// within the time period
				// as numerators or denominators could be missing
				resultsMap = {};
				Object.keys(executions[executionCtr].aggregate_result).forEach(
                function(resultKey) {
	                try {
		                resultKeyObject = JSON.parse(resultKey);
	                } catch (err) {
		                // Result key was not valid JSON
		                // Ignore result
		                resultKeyObject = null;
	                }
	                if ((resultKeyObject !== null)
	                        && (resultKeyObject.date)) {
		                // Result is JSON and has a date value
						// specified

		                // Get data date
		                resultDataDate = moment(parseInt(resultKeyObject.date));

		                // Test if data date is within test
						// period.
		                // Data date is between the start and
						// end of the period,
		                // with granularity
		                // by day inclusive of both start and
						// end
		                if (resultDataDate.isBetween(
			                ...
			                if (resultKeyObject.doctor == doctorPID) {
				                ...
				                if ((resultKeyObject.type == "Ratio")
				                        && (resultKeyObject.type == type)) {
					                // We have a ratio query and
									// that is what we are
									// looking for
					                // Check for previous result
					                previousResult = resultsMap[resultDataDate
					                        .valueOf()];
					                ...
					                if (resultKeyObject.result == "denominator") {
						                ...
						                if (previousResult
							                ...
							                throw "Encountered a second denominator result for Query = "
							                        + queryTitle
							                        + " in execution Timestamp = "
							                        + executions[executionCtr].time
							                        + " Doctor = "
							                        + resultKeyObject.doctor
							                        + " Data Date = "
							                        + resultKeyObject.date;
						                } else {
							                // Save to result
							                ...
							                // Save to map
							                resultsMap[resultDataDate
							                        .valueOf()] = previousResult;
						                }
					                }
				                ...
				                } else if ((resultKeyObject.type == "TenYearRanges")
				                        && (resultKeyObject.type == type)) {
					                // We have a TenYearRanges
									// query and that is what we
									// are looking for
					                // Check for previous result
					                previousResult = resultsMap[resultDataDate
					                        .valueOf()];
					                ...
					                if (previousResult[queryInfo.genderMap[resultKeyObject.gender]]
						                ...
						                throw "Encountered a second result for Query = "
						                        + queryTitle
						                        + " Gender = "
						                        + resultKeyObject.gender
						                        + " which maps to "
						                        + queryInfo.genderMap[resultKeyObject.gender]
						                        + " agesMin = "
						                        + resultKeyObject.agesMin
						                        + " which maps to "
						                ...
						                // Save to result
						                previousResult[queryInfo.genderMap[resultKeyObject.gender]][queryInfo.ageMap[resultKeyObject.agesMin]] = executions[executionCtr].aggregate_result[resultKey];
						                // Save to map
						                resultsMap[resultDataDate
						                        .valueOf()] = previousResult;
					                }
				...
				for (var resultCtr = 0; !chosenResult
				        && resultCtr < resultsList.length; resultCtr++) {
					if (type == "Ratio") {
						if (resultsList[resultCtr].hasOwnProperty("numerator")
						        && resultsList[resultCtr]
						                .hasOwnProperty("denominator")) {
							// We have our result.
							chosenResult = resultsList[resultCtr];
	
							// Save execution time in result
							chosenResult.executionTime = executions[executionCtr].time;
 ...
 * 
 */
function populateRatioQueries(toPopulate, doctor, period, data, earliestExecutionHolder) {
    ...
    queryTitles
		...
		// only process non-excluded ratio type entries
		if (!queryInfo.exclude && (queryInfo.type.toLowerCase() === "ratio")) {
		    // Get values for query for time period
		    var results = extractResult(queryTitle, "Ratio", docInfo.pid, period, data);

		    if (results !== null) {
			    updateEarliestExecution(earliestExecutionHolder, results.executionTime);
 ...
 * @returns {Boolean} whether the object was successfully populated with data
 */
function populateTenYearRanges(toPopulate, xmlPath, doctor, period, data, earliestExecutionHolder) {
    var docInfo = doctors.getDoctorInfo(doctor);
...
/**
 * If the query execution time passed occurs before the execution time in the earliest
 * execution holder passed the earliest execution holder will be updated to the
 * execution time passed.
 * 
 * @param earliestExecutionHolder Object with an executionTime member that can be updated
 * @param executionTime Unix timestamp query execution time that was just used to populate the scorecard 
 */
function updateEarliestExecution(earliestExecutionHolder, executionTime) {
	if (!earliestExecutionHolder.executionTime
	        || (executionTime < earliestExecutionHolder.executionTime)) {

		// We have a new earliest execution time
		earliestExecutionHolder.executionTime = executionTime;
	}
